...
describe('Hud.update() drives exactly the registered set, on the registered bands', () => {
  ...
  it('keeps every row well formed', () => {
    ...
    expect(
      ...
      // Re-measured on this v0.42.0 merge: the ClaudeStone deck builder's
      // render row rides on top of the release's own window and chrome rows.
      // Read off a run of the merged tree.
    ).toEqual({ window: 48, chrome: 85, none: 17 });
