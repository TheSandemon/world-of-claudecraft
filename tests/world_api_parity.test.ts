...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    //
    // The merged interface is the ClaudeStone deck and card-command members on
    // top of the release's own (resolvedAbility from class balance and the four
    // Nythraxis data readouts). Every number here is read off the merged tree.
    expect(IWORLD_MEMBERS.length).toBe(353);
    expect(DATA_MEMBERS.length).toBe(99);
    expect(METHOD_MEMBERS.length).toBe(254);
  });
...
describe('W1: aggregate IWorld member set equals the disjoint union of the facets', () => {
  ...
  it('the facet union equals the pinned IWORLD_MEMBERS set', () => {
    ...
    // Both union pins cover the complete merged contract: the ClaudeStone
    // members, the resolved ability method and the four Nythraxis readouts.
